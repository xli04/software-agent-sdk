"""Helpers for populating ``GuardrailLLMSecurityAnalyzer.safety_experiences``.
...
``GuardrailLLMSecurityAnalyzer(safety_experiences=...)``. Users who want to
inject their own hand-authored experiences can skip these helpers and pass
an arbitrary string directly.

Example:
    >>> from openhands.sdk.security import GuardrailLLMSecurityAnalyzer
    ...
    >>>
    >>> # Default seed
    >>> analyzer = GuardrailLLMSecurityAnalyzer(
    ...     llm=guardrail_llm,
...
def _require_toolshield():
    ...
    try:
        ...
        raise ImportError(
            ...
            "GuardrailLLMSecurityAnalyzer(safety_experiences=...)."
        ) from e
    return toolshield
...
def auto_detect_safety_experiences(
    ...
    Falls back to :func:`default_safety_experiences` when no MCP server is
    detected (unless ``fallback_to_default=False``, in which case returns
    an empty string).
    ...
    logger.warning(
        ...
        f"loading always-active tools only: {active}"
    )
    return load_safety_experiences(active, model=model) if active else ""
