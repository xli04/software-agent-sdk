"""Tests for GuardrailLLMSecurityAnalyzer and toolshield helpers."""
...
from openhands.sdk.security.guardrail_llm_analyzer import (
    GuardrailLLMSecurityAnalyzer,
...
def _make_analyzer(
    ...
    return GuardrailLLMSecurityAnalyzer(
        llm=_make_test_llm(),
...
def _patch_completion(analyzer: GuardrailLLMSecurityAnalyzer, response_or_side_effect):
...
class TestParseRisk:
    ...
    def test_parses_standalone_label(self, text, expected):
        assert GuardrailLLMSecurityAnalyzer._parse_risk(text) == expected
    ...
    def test_inline_label_in_explanation_is_ignored(self):
        ...
        assert (
            GuardrailLLMSecurityAnalyzer._parse_risk(text) == SecurityRisk.HIGH
        )
    ...
    def test_no_label_falls_back_to_high(self):
        ...
        assert (
            GuardrailLLMSecurityAnalyzer._parse_risk("This looks suspicious.")
            == SecurityRisk.HIGH
        )

    def test_empty_text_falls_back_to_high(self):
        assert (
            GuardrailLLMSecurityAnalyzer._parse_risk("") == SecurityRisk.HIGH
        )
...
class TestHistoryWindow:
    ...
    def test_history_window_zero_rejected(self):
        with pytest.raises(ValueError, match="history_window must be >= 1"):
            GuardrailLLMSecurityAnalyzer(llm=_make_test_llm(), history_window=0)
...
class TestToolShieldHelpers:
    ...
    def test_detect_active_mcp_tools_always_includes_terminal(self):
        """Even with no listening ports, terminal-mcp is unconditional."""
        from openhands.sdk.security.toolshield_helpers import (
            ALWAYS_ACTIVE_TOOLS,
            detect_active_mcp_tools,
        )

        # Port map with all unreachable ports
        result = detect_active_mcp_tools(
            host="127.0.0.1",
            port_map={"filesystem-mcp": 1},  # port 1 almost certainly not listening
            timeout=0.1,
        )
        assert "terminal-mcp" in result
        for always_active in ALWAYS_ACTIVE_TOOLS:
