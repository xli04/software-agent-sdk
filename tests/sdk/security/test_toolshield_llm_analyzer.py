...
class TestToolShieldHelpers:
    ...
    def test_auto_detect_returns_empty_when_fallback_disabled_and_no_networked(self):
        """Regression: fallback_to_default=False must return '' when no networked
        MCP server is detected, rather than silently loading terminal-mcp (which
        also requires toolshield to be installed)."""
        from openhands.sdk.security.toolshield_helpers import (
            auto_detect_safety_experiences,
        )

        result = auto_detect_safety_experiences(
            host="127.0.0.1",
            port_map={"filesystem-mcp": 1},  # unreachable
            timeout=0.1,
